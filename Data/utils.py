...
def publish_voxels(map_object, min_dim, max_dim, grid_dims, colors, next_map):
    ...
    # semantic_labels = map_object.global_map[:,3:].detach().cpu().numpy()
    # centroids = map_object.global_map[:, :3].detach().cpu().numpy()
    ...
    # Variance
    totals = total_probs[not_prior].reshape(-1, 1)
    a_tilda = semantic_labels / totals
    variance = a_tilda * (1 - a_tilda) / (1 + totals)
    low_variance = variance < 0.01
    centroids = centroids[low_variance[:, 0], :]
    semantic_labels = semantic_labels[low_variance]
