...
# Local Mapping by applying BKI recursively
class LocalMap_BKI(ConvBKI):
    def __init__(self, grid_size, min_bound, max_bound, weights, filter_size, num_classes=21, prior=0.001, device="cpu",
                 datatype=torch.float32, sparse=True, delete_time=10):
        super().__init__(grid_size, min_bound, max_bound, filter_size=filter_size,
                 num_classes=num_classes, prior=prior, device=device, datatype=datatype)
    ...
    def reset_grid(self):
        self.local_map = self.initialize_grid()
        self.current_pose = None

    # Uses saved weights instead of generating a filter
    def update_map(self, semantic_preds):
        semantic_preds = semantic_preds.to(self.dtype)
        # Apply transform to old positions
        old_centroids = torch.matmul(self.relative_transform[:3, :3], self.centroids.T).T + self.relative_transform[:3, 3]
        # Apply normalization
        old_states = 1/(self.filter_size**3) * self.local_map.view(old_centroids.shape[0], self.num_classes)
        old_preds = torch.hstack((old_centroids, old_states))

        # Concatenate old centroids with new points
        N, C = semantic_preds.shape
        if C != self.num_classes + 3:
            o_h_preds = semantic_preds[:, 3].long()
            c_preds = F.one_hot(o_h_preds, num_classes=self.num_classes)
            semantic_preds = torch.hstack((semantic_preds[:, :3], c_preds))

        all_preds = torch.vstack((old_preds, semantic_preds))

        # Change to indices using our global frame bounds
        grid_pc = self.grid_ind(all_preds)

        # Update local map
        update = torch.zeros_like(self.local_map, requires_grad=False)
        update = self.add_to_update(update, grid_pc, True)
    ...
    def propagate(self, pose):
        ...
        to_ego = torch.linalg.inv(pose)
        self.relative_transform = torch.matmul(to_ego, self.current_pose)
        self.current_pose = pose
    ...
    def label_points(self, points):
        ...
        labels = torch.zeros((points.shape[0], self.num_classes), dtype=torch.float32, device=self.device)
        local_mask = torch.all((points < self.max_bound) & (points >= self.min_bound), dim=1)

        local_points = points[local_mask]

        grid_inds = torch.floor((local_points - self.min_bound) / self.voxel_sizes)
        maxes = (self.grid_size - 1).view(1, 3)
        clipped_inds = torch.clamp(grid_inds, torch.zeros_like(maxes), maxes).to(torch.long)

        labels[local_mask, :] = self.local_map[clipped_inds[:, 0], clipped_inds[:, 1], clipped_inds[:, 2], :]
        labels[~local_mask, :] = self.prior
...
class LocalMap(ConvBKI):
    def __init__(self, grid_size, min_bound, max_bound, weights, filter_size, num_classes=21, prior=0.001, device="cpu",
                 datatype=torch.float32, sparse=True, delete_time=1):
        super().__init__(grid_size, min_bound, max_bound, filter_size=filter_size,
                 num_classes=num_classes, prior=prior, device=device, datatype=datatype)
    ...
    def reset_grid(self):
        ...
        self.map_times = None
        self.initial_pose = None
        self.translation_discretized = torch.zeros(3).to(self.device)
        self.points_rotation = torch.eye(3, dtype=self.dtype, device=self.device)
        self.points_translation = torch.zeros(3, dtype=self.dtype, device=self.device)
    ...
    def get_local_map(self, min_bound=None, max_bound=None):
        ...
        if max_bound is None:
            max_bound = self.max_bound
        local_min_bound = min_bound + self.voxel_translation
        local_max_bound = max_bound + self.voxel_translation
        if self.global_map is not None:
            inside_mask = self.inside_mask(local_min_bound, local_max_bound)
            allocated_map = torch.tensor(self.global_map[inside_mask], device=self.device, dtype=self.dtype)
            grid_map = self.grid_ind(allocated_map, min_bound=local_min_bound, max_bound=local_max_bound)
            grid_indices = grid_map[:, :3].to(torch.long)
            local_map[grid_indices[:, 0], grid_indices[:, 1], grid_indices[:, 2], :] = allocated_map[:, 3:]
    ...
    def update_map(self, semantic_preds):
        ...
        updated_centroids = self.centroids[updated_cells, :] + self.voxel_translation
        local_values = local_map.view(-1, self.num_classes)[updated_cells]
        new_cells = torch.cat((updated_centroids, local_values), dim=1)

        self.global_map = new_cells
        return self.global_map
    ...
    def propagate(self, pose):
        ...
        relative_translation = pose[:3, 3] - self.initial_pose[:3, 3]
        # To select voxels from memory, find the nearest voxel
        self.voxel_translation = torch.round(relative_translation / self.voxel_sizes) * self.voxel_sizes
        self.nearest_voxel = self.initial_pose[:3, 3] + self.voxel_translation
