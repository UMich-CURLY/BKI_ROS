...
public:
    ...
    bool saveMapService(liorf::save_mapRequest& req, liorf::save_mapResponse& res)
    {
      ...
      //   cout << "Vector length: " << cloudKeyPoses3D->size() << " timestamps: " << timeStamps.size() << endl;


      cout << "TEST" << endl;
      rosbag::Bag bag;
      bag.open("/home/josh/Downloads/rellisLidarPoses.bag", rosbag::bagmode::Write);
      ros::Publisher cloud_pub = nh.advertise<sensor_msgs::PointCloud2>("map_pointcloud", 1);
      ros::Publisher pose_pub = nh.advertise<geometry_msgs::PoseStamped>("map_pose", 1);


      for (int i = 0; i < (int)surfCloudKeyFrames.size(); i++) {
            cout << "TEST " <<  (int)surfCloudKeyFrames.size()<< endl;

            // Publish point cloud on "map_pointcloud" topic
            sensor_msgs::PointCloud2 cloud_msg;
            pcl::toROSMsg(*surfCloudKeyFrames[i], cloud_msg);
            cloud_msg.header.stamp = timeStamps[i]; // Set the timestamp to the current time
            cloud_msg.header.frame_id = "map";
            cloud_pub.publish(cloud_msg);

            // Publish pose on "map_pose" topic
            Eigen::Affine3f pose = pcl::getTransformation(cloudKeyPoses6D->points[i].x, cloudKeyPoses6D->points[i].y, cloudKeyPoses6D->points[i].z, cloudKeyPoses6D->points[i].roll, cloudKeyPoses6D->points[i].pitch, cloudKeyPoses6D->points[i].yaw);
            geometry_msgs::PoseStamped pose_msg;
            pose_msg.header.stamp = timeStamps[i]; // Set the timestamp to the current time
            ...
            cout << cloudKeyPoses6D->points[i].x << " " << pose_msg.pose.position.x << pose_msg.pose.orientation.x << pose_msg.pose.orientation.y<<pose_msg.pose.orientation.z<<pose_msg.pose.orientation.w<<cloudKeyPoses6D->points[i].roll<<cloudKeyPoses6D->points[i].pitch<<cloudKeyPoses6D->points[i].yaw;
            // Write to bag file
            bag.write("map_pointcloud", timeStamps[i], cloud_msg);
            bag.write("map_pose", timeStamps[i], pose_msg);

            cout << "\r" << std::flush << "Saving frame " << i << " points: " << cloud_msg.width * cloud_msg.height <<  " of " << surfCloudKeyFrames.size() << " ...";
      }
...
/*             float arx = (crx*sry*srz*pointOri.x + crx*crz*sry*pointOri.y - srx*sry*pointOri.z) * coeff.x
    ...
    void saveKeyFramesAndFactor()
    {
        ...
        timeStamps.push_back(timeLaserInfoStamp);
