...
if DATA_CONFIG == "Carla":
    ...
    test_ds = KITTIDataset(test_dir, data_params["eval_params"], num_frames=T, remap=remap, get_gt=False, bev=False)
...
with torch.no_grad():
    ...
    for horizon_batch, pose_batch, output_batch in dataloader_test:
        ...
        for t in range(len(all_points[b])):
            ...
            if "Transfer" in MODEL_CONFIG:
                new_labels = torch.zeros_like(all_labels)[:, :data_params["num_classes"]]
                labels_dict = test_ds.labels_dict
                for i in labels_dict.keys():
                    if len(labels_dict[i]) == 0:
                        continue
                    temp_tensor = torch.vstack([all_labels[:, i] for i in labels_dict[i]])
        ...
        with torch.no_grad():
            measure_inf_time(model, [current_map, labeled_pc_torch])
        preds = current_map
