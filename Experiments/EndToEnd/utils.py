...
def load_model(model_params, dev):
    ...
    if "KITTI" in model_params["seg_path"]:
        seg_net = SPVCNN(
            num_classes=19,
            cr=model_params["cr"],
    ...
    if "KITTI" in model_params["seg_path"]:
        e2e_net = BKINet(seg_net, bki_layer, prop_net, grid_size, device=dev, num_classes=num_classes, remap=True)
    else:
        e2e_net = BKINet(seg_net, bki_layer, prop_net, grid_size, device=dev, num_classes=num_classes, remap=False)
    return e2e_net
...
def sigmoid(x, width=0.01, offset=-10):
    x = x/width + offset
