...
class BKINet(torch.nn.Module):
    ...
    def forward(self, input_data):
        ...
        if self.noise > 0:
            k = int(point_labels.size(0) * self.noise)
            perm = torch.randperm(point_labels.size(0))
            corrupt_indices = perm[:k]
            rand_labels = torch.randint(11, (corrupt_indices.shape[0], ))
            corrupt_preds = F.one_hot(rand_labels, 11)
            point_labels[corrupt_indices, :] = corrupt_preds.float().cuda()
