...
def make_dataset() -> Dataset:
    if configs.dataset.name == 'semantic_kitti':
        ...
        dataset = nuScenes(root=configs.dataset.root,
                                num_points=configs.dataset.num_points,
                                voxel_size=configs.dataset.voxel_size)
    elif configs.dataset.name == 'carla':
        from core.datasets import Carla
        dataset = Carla(root=configs.dataset.root,
                                num_points=configs.dataset.num_points,
                                voxel_size=configs.dataset.voxel_size)
    elif configs.dataset.name == 'rellis':
        from core.datasets import Rellis
        dataset = Rellis(root=configs.dataset.root,
                                num_points=configs.dataset.num_points,
                                voxel_size=configs.dataset.voxel_size)
    else:
