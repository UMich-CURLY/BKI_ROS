...
```bash
torchpack dist-run -np [num_of_gpus] python evaluate.py configs/semantic_kitti/MotionNet.yaml --name [num_of_net]
```

For example, to test the model `SemanticKITTI_val_SPVNAS@65GMACs` on one GPU, you may run

```bash
torchpack dist-run -np 1 python evaluate.py configs/semantic_kitti/MotionNet.yaml --name SemanticKITTI_val_SPVNAS@65GMACs
```
